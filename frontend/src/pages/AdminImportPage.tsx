...
export default function AdminImportPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 py-10 space-y-6">
      <div>
        <p className="text-[10px] font-black uppercase tracking-[0.25em] text-[color:var(--color-data)] mb-1">Admin</p>
      ...
      <section className="rounded-lg p-5 space-y-3" style={{ background: 'var(--color-surface)', border: '1px solid var(--color-line)' }}>
        ...
        <input
          ...
          className="w-full px-3 py-2 rounded bg-black/30 border border-white/10 text-sm text-white/90 focus:outline-none focus:border-[color-mix(in_srgb,var(--color-data)_60%,transparent)]"
        />
      ...
      <section className="rounded-lg p-5 space-y-3" style={{ background: 'var(--color-surface)', border: '1px solid var(--color-line)' }}>
        ...
        <input
          ...
          className="block w-full text-sm text-white/70 file:mr-3 file:py-2 file:px-4 file:rounded file:border-0 file:text-xs file:font-black file:uppercase file:tracking-widest file:bg-[color-mix(in_srgb,var(--color-data)_20%,transparent)] file:text-[color:var(--color-data)] hover:file:bg-[color-mix(in_srgb,var(--color-data)_30%,transparent)]"
        />
      ...
      {result && (
        <div className="rounded-lg p-5 space-y-3" style={{ background: 'rgba(255, 184, 0, 0.06)', border: '1px solid rgba(255, 184, 0, 0.4)' }}>
          <p className="text-xs font-black uppercase tracking-widest text-[color:var(--color-data)]">导入成功</p>
          ...
          <div className="grid grid-cols-5 gap-2 text-center text-xs">
            {([
              ...
              <div key={k} className="p-2 rounded bg-black/30">
                <div className="text-base font-black text-[color:var(--color-data)]">{result.counts[k]}</div>
                <div className="text-[10px] text-white/40 mt-0.5">{label}</div>
              </div>
            ))}
          </div>
          <div className="flex gap-3 pt-2 text-xs">
            <a href={`/matches/${result.matchId}`} className="text-[color:var(--color-data)] underline">查看比赛 →</a>
          </div>
        </div>
      )}
    </div>
